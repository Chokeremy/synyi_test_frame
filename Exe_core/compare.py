...
def except_check(check_value, key_value, db_config, inter_value):
    # 判断是否要依赖其他接口返回数据
    if re.findall('\$var{|\$sheet\d+.var{', check_value, re.I):
        check_value = check_var(check_value, key_value, inter_value)
    ...
    if re.findall('\$sql{[\s\S]+?}', check_value, re.I):
        ...
        check_value = check_select(check_value, 'oracle', db_config['oracle'])

    elif re.findall('\$mssql{[\s\S]+?}', check_value, re.I):
        check_value = check_select(check_value, 'sqlserver', db_config['sqlserver'])

    return check_value


def compare(check_params, response, key_value, db_config, inter_value):
    """
    校验结果判断
    :param inter_value:
    :param db_config:
    ...
    for param in params_list:
        ...
        # 获取期望值
        except_value = except_check(check_value, key_value, db_config, inter_value)
