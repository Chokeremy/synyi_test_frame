...
class Execute(RequestInterface):
    ...
    def execute_case(self, all_cases, inter_value, db_config, timeout, path):
        """
        逐条执行用例
        :param timeout:
        :param db_config:
        ...
        for index, sheet_cases in enumerate(all_cases):
            ...
            for case in sheet_cases['sheet_value']:
                ...
                # url = case['url']
                url = check_sort(case['url'], key_value, db_config, inter_value=inter_value)
                if "synyi_test_frame_track" in url:
                ...
                header = check_sort(case['header'], key_value, db_config, inter_value=inter_value)
                ...
                param = check_sort(case['param'], key_value, db_config, inter_value=inter_value)
                ...
                body = check_sort(case['body'], key_value, db_config, inter_value=inter_value, case_path=path)
                ...
                # 判断请求之前的操作
                if case['exe_before']:
                    exe_before = check_sort(case['exe_before'], key_value, db_config)
                ...
                if response['status'] == case['status_except']:
                    ...
                    if case['check_before']:
                        check_before = check_sort(case['check_before'], key_value, db_config)
                    ...
                    # if response['data']:
                    #     if case['checks']:
                    #         result = compare(case['checks'], response['data'], key_value, db_config, inter_value)
                    #     else:
                    ...
                    # 判断断言之后的操作
                    if case['check_after']:
                        check_after = check_sort(case['check_after'], key_value, db_config)
                ...
                sy_log.logger.info("---------------------------------------------------------\n")
                test_result_list.append(test_result)
            inter_value[index]['inter_value'] = key_value
            ...
            if 'fail' in test_result_list:
                sheet_result.append({'sheet_name': sheet_name, 'result': result_value, 'fail': True})
            else:
                sheet_result.append({'sheet_name': sheet_name, 'result': result_value, 'fail': False})
            sy_log.logger.info(
