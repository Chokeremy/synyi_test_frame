...
def make_zip(output_filename, source_dir=None, source_file=None):
    """
    :param source_dir:
    ...
    if source_dir:
        ...
        zipf.write(source_file, source_dir)
    zipf.close()
...
def report_data(data):
    ...
    for r in sheet_result:
        ...
        if r['fail']:
            fail_sheet_name.append(r['sheet_name'])
    ...
    value = {"sum_case": sum_case, "success_case_num": success_case_num, "case_list": case_list,
             "sheet_result": sheet_result, "sheet_name": sheet_name, "fail_sheet_name": fail_sheet_name}
    return value
...
class Report(object):
    ...
    生成测试日报
    """
    def __init__(self, report_path, data, info):
    ...
    def totali(self):
        ...
        fail_sheet_name = self.data['fail_sheet_name']
        fsn = ','.join(fail_sheet_name) if len(fail_sheet_name) != 0 else ''
        ...
        self.write_center(self.worksheet, "A3", '项目名称', self.workbook)
        self.write_center(self.worksheet, "A4", '接口版本', self.workbook)
        self.write_center(self.worksheet, "A5", '脚本语言', self.workbook)
        self.write_center(self.worksheet, "A6", '测试人员', self.workbook)

        self.write_center(self.worksheet, "C3", "接口总数", self.workbook)
        self.write_center(self.worksheet, "C4", "通过总数", self.workbook)
        self.write_center(self.worksheet, "C5", "失败总数", self.workbook)
        self.write_center(self.worksheet, "C6", "测试日期", self.workbook)
        self.write_center(self.worksheet, "E3", "接口通过率", self.workbook)
        self.worksheet.merge_range('E4:E6', '=TEXT(D4/D3,"0.00%")', self.get_format_center(self.workbook))
        self.write_center(self.worksheet, 'F3', "失败sheet页", self.workbook)

        self.write_center(self.worksheet, "B3", self.info['project_name'], self.workbook)
        self.write_center(self.worksheet, "B4", self.info['version'], self.workbook)
        self.write_center(self.worksheet, "B5", "python", self.workbook)
        self.write_center(self.worksheet, "B6", self.info['tester'], self.workbook)
        self.write_center(self.worksheet, "D3", sumCase, self.workbook)
        self.write_center(self.worksheet, "D4", success_case_num, self.workbook)
        self.write_center(self.worksheet, "D5", sumCase - success_case_num, self.workbook)
        self.write_center(self.worksheet, "D6", nowTime, self.workbook)
        self.worksheet.merge_range('F4:F6', fsn, self.get_format_center(self.workbook))
        self.pie()
...
if __name__ == '__main__':
    source_dir = r'D:\SynyiWork\synyi_test_frame\Exe_core'
    output_filename = r'D:\SynyiWork\synyi_test_frame\Result\asdds.zip'
