...
class ReadCaseInfo(object):
        ...
        if case_suites:
            ...
            for suite in case_suites[0]:
                if suite in self.excel.sheet_names():
                    ...
                    sheet_value = {'sheet_number': i, 'sheet_name': suite, 'sheet_values': sheet._cell_values[1:]}
                    i += 1
            ...
            if self.excel.nsheets < 2:
                ...
                sheet_cases = [{'sheet_number': i+1, 'sheet_name': sheets[i].name, 'sheet_values': sheets[i]._cell_values[1:]} for
    ...
    def project_info(self):
        ...
        send_report = self.sheet_config.cell_value(7, 1)
        if send_report:
            try:
                send_report = json.loads(send_report)
            except Exception:
                sy_log.logger.info("序列化失败，无法发送消息")
                sys.exit(0)
            project_info["send_info"] = send_report
        return project_info
    ...
    def pretreated_case(self, *case_suites):
        ...
        for sheet_cases in all_case_values:
            ...
            sheet_case_values = []
            sheet_inter_values = {'sheet_number': sheet_cases['sheet_number'], 'inter_value': {}}
